...
<div class="buttons">
	...
	{if $model->id && $active_worker->hasPriv('contexts.cerberusweb.contexts.datacenter.domain.delete')}<button type="button" onclick="$(this).parent().siblings('fieldset.delete').fadeIn();$(this).closest('div').fadeOut();"><span class="glyphicons glyphicons-circle-remove" style="color:rgb(200,0,0);"></span> {'common.delete'|devblocks_translate|capitalize}</button>{/if}
</div>
